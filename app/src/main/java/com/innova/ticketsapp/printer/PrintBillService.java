package com.innova.ticketsapp.printer;
...
import com.innova.ticketsapp.MainActivity;
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if(context.equals(STR_PRNT_BILL)){	// print bill
			...
			ticket=new JSONObject(context);
				...
				options.inSampleSize = calculateInSampleSize(options, 5, 5);
				options.inJustDecodeBounds = false;
				Bitmap logo=BitmapFactory.decodeByteArray(base64converted,0,base64converted.length,options);
				printer.prn_drawBitmap(logo, 5, height);
				height += (140);


				for(int i=0;i<ticket.getJSONArray("encabezado").length();i++){
					line = ticket.getJSONArray("encabezado").get(i).toString()+"\n";
					//list.add(PrintableString.emptyLine(1));
					System.err.println(line);
					printer.prn_drawTextEx(line, 5, height, 384, -1, fontName, fontSize, 0, fontStyle, 1);
					height += (fontSize+5);
				}

				line = "\n";
				printer.prn_drawTextEx(line, 5, height, 384, -1, fontName, 23, 0, fontStyle, 1);
				height += (32);

				line = "Cuenta:" +ticket.getString("cuenta")+"\n";
				printer.prn_drawTextEx(line, 5, height, 384, -1, fontName, 23, 0, fontStyle, 1);
				height += (29);
				...
				printer.prn_drawLine(5, height, 384, height, 5);
				height += (6);
				...
				printer.prn_drawTextEx(line, 5, height, 384, -1, fontName,25, 0, fontStyle, 1);
				height += (31);
				...
				for(int i=0;i<ticket.getJSONArray("cuerpo").length();i++){
					...
					printer.prn_drawTextEx(line, ((384-(line.length()*11))/2), height, 384, -1, fontName,23, 0, fontStyle, 1);
					height += (29);
				}
...
//            ret +=printer.prn_drawTextEx(context, 300, ret,-1,-1, "arial", 25, 1, 0, 0);
        ...
        Intent i = new Intent(MainActivity.PRNT_ACTION);
        i.putExtra("ret", ret);
...
//		printer.prn_drawLine(32, 423, 352, 422, 32);
